    ...
    <div className="space-y-3 p-2">
      {items.map((item) => (
        ...
        >
          ...
          <div className={`absolute top-0 right-0 w-24 h-24 rounded-full filter blur-3xl opacity-10 ${item.type === RecordType.NEWS ? 'bg-blue-500' : 'bg-rose-500'}`} />
          ...
          <div className="flex justify-between items-start gap-5 relative z-10">
            <div className={`mt-1 p-2 rounded-xl shrink-0 ${
                item.type === RecordType.NEWS ? 'bg-blue-500/10 text-blue-400' : 'bg-rose-500/10 text-rose-400'
                 ...
                 {item.type === RecordType.NEWS ? <FileText size={20} /> : <ShieldAlert size={20} />}
            </div>
